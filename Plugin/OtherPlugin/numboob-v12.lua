...
function getTableContent(obj) -- 用于打印玩家列表
    ...
    local isArray = function(arr)
        local count = 0
        for k, v in pairs(arr) do count = count + 1 end
        for i = 1, count do if arr[i] == nil then return false end end
        return true, count
    end
    dumpObj = function(obj, level)
        if type(obj) ~= "table" then return wrapVal(obj) end
        level = level + 1
        local tokens = {}
        tokens[#tokens + 1] = "玩家列表:\n"
        local ret, count = isArray(obj)
        if ret then
            for i = 1, count do tokens[#tokens + 1] = getIndent(level) .. wrapVal(obj[i], level) .. "" end
        else
...
function gameprepare(msg)      -- 游戏初始化（开始）
    if #msg.fromGroup < 2 then -- 判断是否是私聊环境,注：必须有引号否则将被视为nil
        if getUserConf(msg.fromQQ, "Stage", 0) ~= 0 then
            ...
            local key = ranint(1, getUserConf(msg.fromQQ, "boobrange", 300))
            setUserConf(msg.fromQQ, "boobkey", key)
            sendMsg("游戏开始\n本轮游戏范围：1 —— " .. getUserConf(msg.fromQQ, "boobrange", 300)
                .. " \n请在游戏结束后使用【数字指定key】来设置范围，范围将被记录为默认值，无须重复设置。例：数字指定1000。{FormFeed}单人游戏无回答限时，现在可以直接开始回答数字。结束请发送【数字炸弹结束】。",
...
function gamestart(msg)
    if #msg.fromGroup < 2 then -- 检查是否私聊
                ...
                if max ~= min then
                    ...
                    for t = 1, 200 do -- 等待回答模块，如果回答则跳出
                        if getGroupConf(msg.fromGroup, "isanswer", 0) ~= 1 and getGroupConf(msg.fromGroup, "Stage", 0) ~= 0 then
                            sleepTime(100)
                            setGroupConf(msg.fromGroup, "answertime", t)
                        else
                            break
                        end
                    end
                    ...
                    return "没得可选了，看来这炸弹 「" .. integer(getGroupConf(msg.fromGroup, "nbkey"))
                        .. " 」 [CQ:at,id=" .. getGroupConf(msg.fromGroup, "boobchecker")
                        .. "] 是要吃下去了。"
                end
            end
...
function gameanswer(msg)
    if msg.fromGroup == "" then -- 私聊执行模块
        if getUserConf(msg.fromQQ, "Stage", 0) == 0 then return "{nick}还没有开始游戏呢。" end
        local answer = tonumber(getnum(msg.fromMsg))
        ...
        if answer == key then
            ...
            sendMsg("！！炸啦！" .. "\n{nick}一共活过了" .. getUserConf(msg.fromQQ, "NumboobTimes", 1)
                .. "轮。", msg.fromGroup, msg.fromQQ)
        ...
        if answer > max or answer < min then -- 检查是否合法
            ...
            setGroupConf(msg.fromQQ, "，boobanswer", answer)
            if answer < key then -- 若合法，修正上下限。
                setGroupConf(msg.fromQQ, "boobmin", answer + 1)
            else
                setGroupConf(msg.fromQQ, "boobmax", answer - 1)
            end
    ...
    if msg.fromQQ == getGroupConf(msg.fromGroup, "boobchecker") then -- 判断轮次是否正确
        ...
        if answer == key then -- 判断是否爆炸
            setGroupConf(msg.fromGroup, "Stage", 0)
            return "炸弹爆炸！游戏结束。\n{nick}({at})引爆了数字炸弹。炸弹为"
                .. integer(getGroupConf(msg.fromGroup, "nbkey"))
        end
        if answer > max or answer < min then -- 检查是否合法
            return "这个数不在范围里诶，这样不行的，重新说！\n范围是" ..
                integer(min) .. " 到 " .. integer(max)
        else                                                  -- 合法回答
            setGroupConf(msg.fromGroup, "boobanswer", answer) -- 保存答案
            setGroupConf(msg.fromGroup, "isanswer", 1)
            local key = tonumber(getGroupConf(msg.fromGroup, "nbkey"))
            if answer < key then -- 修正上下限范围
                setGroupConf(msg.fromGroup, "boobmin", answer + 1)
            else
                setGroupConf(msg.fromGroup, "boobmax", answer - 1)
            end
        end
    else
        return "还没到{nick}，别捣乱！"
