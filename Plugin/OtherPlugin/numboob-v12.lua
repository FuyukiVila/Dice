...
function gameprepare(msg)      --游戏初始化（开始）
    if #msg.fromGroup < 2 then --判断是否是私聊环境,注：必须有引号否则将被视为nil
        if getUserConf(msg.fromQQ, "Stage", 0) ~= 0 then
            ...
            sendMsg(
                "游戏开始\n本轮游戏范围：1 —— " ..
                getUserConf(msg.fromQQ, "boobrange", 300) ..
...
function gamestart(msg)
    ...
    if getGroupConf(msg.fromGroup, "Stage", 0) ~= 1 then --若不在私聊视为群里，直接检查游戏进程信息
        ...
        sendMsg("本轮数字范围已确定为 0 至 " .. Numrange .. "\n" .. getTableContent(Playerlist), msg
            .fromGroup, msg.fromQQ)
        ---主题轮询
            ...
            for i = 1, #Playerlist do
                ...
                if max - min > 2 then
                    ...
                    return "没得可选了，看来这炸弹 「" ..
                        ...
                        " 」 [CQ:at,id=" .. getGroupConf(msg.fromGroup, "boobchecker") .. "] 是要吃下去了。"
                end
            end
...
function gameanswer(msg)
    if #msg.fromGroup < 2 then --私聊执行模块
        if getUserConf(msg.fromQQ, "Stage", 0) == 0 then
        ...
        if answer == key then
            ...
            sendMsg("！！炸啦！" .. "\n{nick}一共活过了" .. getUserConf(msg.fromQQ, "NumboobTimes", 1) .. "轮。",
                msg.fromGroup, msg.fromQQ)
