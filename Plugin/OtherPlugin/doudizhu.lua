...
help_info = {      --自定义帮助
    ...
    "  发牌：发送“" .. chu_pai .. "”，特别的，如果没选牌，可以直接发送“" ..
    chu_pai .. " 牌1 牌2 牌n”出牌",
    ...
    if (cc == 2 and ctpc[1] ~= 12) then  --先手（其余两位要不起且牌型正确）
        ...
        if (#(getUserConf(msg.fromQQ, "my_puke_card")) == 0) then --检测到牌出完了,重置
            ...
            if (#(getUserConf(msg.fromQQ, "my_puke_card")) > 5) then
                ...
                return "{nick}出牌：" ..
                    toCardType(tpc[1]) ..
                    table.concat(real_puke, " ") .. "\f{nick}还剩" ..
                    #(getUserConf(msg.fromQQ, "my_puke_card")) .. "张牌了"
            end
        end
    elseif (cc ~= 2) then                --后手
        if (isCardRight(tpc, ctpc)) then --牌型点数大于上家且正确
            ...
            if (#(getUserConf(msg.fromQQ, "my_puke_card")) == 0) then --检测到牌出完了,重置
                ...
                if (#(getUserConf(msg.fromQQ, "my_puke_card")) > 5) then
                    ...
                    return "{nick}出牌：" ..
                        toCardType(tpc[1]) ..
                        table.concat(real_puke, " ") .. "\f{nick}还剩" .. #(getUserConf(msg.fromQQ, "my_puke_card")) ..
                        "张牌了"
                end
            end
        end
